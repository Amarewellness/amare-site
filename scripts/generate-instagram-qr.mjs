/**
 * AMARÉ Instagram QR → https://www.instagram.com/954active?igsh=aHl3Yno3dmE3aXpl
 * Taupe-on-cream palette (matches in-studio print reference).
 * Run: node scripts/generate-instagram-qr.mjs
 */
...
const URL = "https://www.instagram.com/954active?igsh=aHl3Yno3dmE3aXpl";

/** Warm taupe + cream — sampled from studio Instagram QR reference */
const COLORS = {
  dot: "#a08769",
  corner: "#a08769",
  bg: "#faf6f0",
...
async function main() {
  ...
  const pngPath = path.join(OUT_DIR, "qr-instagram-954active.png");
  const svgPath = path.join(OUT_DIR, "qr-instagram-954active.svg");
...
<svg xmlns="http://www.w3.org/2000/svg" width="${OUTER}" height="${OUTER}" viewBox="0 0 ${OUTER} ${OUTER}">
  ...
  console.log("Instagram QR generated:");
  console.log("  URL:", URL);
